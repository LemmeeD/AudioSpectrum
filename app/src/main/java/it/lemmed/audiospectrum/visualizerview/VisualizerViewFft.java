...
public class VisualizerViewFft extends View implements VisualizerView {
    //FIELDS
    protected int id;
    ...
    //CONSTRUCTORS
    public VisualizerViewFft(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    //Dall'interfaccia creata, ma utile anche per View
    @Override
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //
        /*
        rect.set(0, getHeight()/4, getWidth(), (3*getHeight())/4);
        float maxHeight = getHeight()/4;
        float[] points = new float[bytes.length];
        for (int i = 0; i < bytes.length - 1; i++) {
            //points[i] = (float) bytes[i];
        }
        ...
        */
        ...
        /*
        float[] magnitude = returnMagnitude(bytes);
        float[] phase = returnPhase(bytes);
        float coeffX = ((float) getWidth()) / ((float) magnitude.length);
        float coeffY = ((float) getHeight()) / 512f;
        for (int i = 0; i < magnitude.length; i++) {
            canvas.drawPoint(i*coeffX, (getHeight()/2f)-coeffY*magnitude[i], this.paint);
            canvas.drawPoint(i*coeffX, (3*getHeight()/4f)-coeffY*phase[i], this.paint);
        }
        */
        ...
        float[] magnitude = returnMagnitude(bytes);
        float[] phase = returnPhase(bytes);
        float coeffX = ((float) getWidth()) / ((float) magnitude.length-1);
        float coeffY = ((float) getHeight()) / 512f;
        for (int i = 0; i < (magnitude.length-1); i++) {
            canvas.drawLine(i*coeffX, getHeight()/2f-coeffY*magnitude[i], (i+1)*coeffX, (getHeight()/2f)-coeffY*magnitude[i+1], this.paint);
            canvas.drawLine(i*coeffX, (3*getHeight()/4f)-coeffY*phase[i], (i+1)*coeffX, (3*getHeight()/4f)-coeffY*phase[i+1], this.paint);
        }
    ...
    private static float[] returnMagnitude(byte[] bytes) {
        int n = bytes.length;
        ...
        for (int k = 1; k < n / 2; k++) {
            int i = k * 2;
            magnitudes[k] = (float) Math.hypot(bytes[i], bytes[i + 1]);     //module (form) operation
        }
        return magnitudes;
    }

    public static float[] returnPhase(byte[] bytes) {
        int n = bytes.length;
        float[] magnitudes = new float[n / 2 + 1];
