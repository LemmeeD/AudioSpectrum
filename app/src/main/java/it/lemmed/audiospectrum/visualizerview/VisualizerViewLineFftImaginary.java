...
public class VisualizerViewLineFftImaginary extends View implements VisualizerView {
    //FIELDS
    protected int id;
    ...
    //CONSTRUCTORS
    public VisualizerViewLineFftImaginary(Context context, float strokeWidth, int color) {
        super(context);
    ...
    public VisualizerViewLineFftImaginary(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.doneGettingOriginalSize = false;
        init();
        //applyCustomAttributes(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //Disegno vero
        float[] imag = returnImag(bytes);
        float coeffX = ((float) getWidth()) / ((float) imag.length-1);
        float coeffY = ((float) getHeight()) / 256f;
        for (int i = 0; i < (imag.length-1); i++) {
            canvas.drawLine(i*coeffX, coeffY*(128 - imag[i]), (i+1)*coeffX, coeffY*(128 - imag[i+1]), this.paint);
        }
    ...
    //imag(byte): [-128, 128]
    private static float[] returnImag(byte[] bytes) {
        int n = bytes.length;
        float[] imag = new float[(n / 2) + 1];
        imag[0] = (float) bytes[0];
        imag[n / 2] = (float) bytes[1];
        for (int k = 1; k < n / 2; k++) {
            int i = (k * 2) + 1;
            imag[k] = (float) bytes[i];
        }
        return imag;
    }

    //symmetry is wrong probably...
    private static float[] returnImagWithSymmetry(byte[] bytes) {
        int n = bytes.length;
        float[] imag = new float[n];
        imag[0] = imag[n - 1] = (float) bytes[0];
        imag[n / 2] = (float) bytes[1];
        for (int k = 1; k < n / 2; k++) {
            int i = (k * 2) + 1;
            imag[k] = (float) bytes[i];
