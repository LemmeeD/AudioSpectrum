...
public class VisualizerViewLineFftPhase extends View implements VisualizerView {
    //FIELDS
    protected int id;
    ...
    //CONSTRUCTORS
    public VisualizerViewLineFftPhase(Context context, float strokeWidth, int color) {
        super(context);
    ...
    public VisualizerViewLineFftPhase(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.doneGettingOriginalSize = false;
        init();
        //applyCustomAttributes(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float coeffX = ((float) getWidth()) / ((float) phase.length-1);
        float coeffY = ((float) getHeight()) / ((float) 4*((float) Math.PI));
        for (int i = 0; i < (phase.length-1); i++) {
            canvas.drawLine(i*coeffX, coeffY*(((float) 2*((float) Math.PI)) - phase[i]), (i+1)*coeffX, coeffY*(((float) 2*((float) Math.PI)) - phase[i+1]), this.paint);
        }
    ...
    //symmetry probably wrong...
    public static float[] returnPhaseWithSymmetry(byte[] bytes) {
        int n = bytes.length;
        float[] phases = new float[n];
        phases[0] = phases[n / 2] = phases[n -1] =  0;
        for (int k = 1; k < n / 2; k++) {
            int i = k * 2;
            phases[k] = (float) Math.atan2(bytes[i + 1], bytes[i]);
            phases[(n-1) - k] = - phases[k];
        }
        return phases;
    }
