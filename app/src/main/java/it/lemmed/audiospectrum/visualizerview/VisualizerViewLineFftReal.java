...
public class VisualizerViewLineFftReal extends View implements VisualizerView {
    //FIELDS
    protected int id;
    ...
    //CONSTRUCTORS
    public VisualizerViewLineFftReal(Context context, float strokeWidth, int color) {
        super(context);
    ...
    public VisualizerViewLineFftReal(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.doneGettingOriginalSize = false;
        init();
        //applyCustomAttributes(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float coeffX = ((float) getWidth()) / ((float) reals.length-1);
        float coeffY = ((float) getHeight()) / 256f;
        for (int i = 0; i < (reals.length-1); i++) {
            canvas.drawLine(i*coeffX, coeffY*(128 - reals[i]), (i+1)*coeffX, coeffY*(128 - reals[i+1]), this.paint);
        }
