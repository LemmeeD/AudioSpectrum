...
public class VisualizerViewPointFftImaginary extends View implements VisualizerView {
    //FIELDS
    protected int id;
    ...
    //CONSTRUCTORS
    public VisualizerViewPointFftImaginary(Context context, float strokeWidth, int color) {
        super(context);
    ...
    public VisualizerViewPointFftImaginary(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.doneGettingOriginalSize = false;
        init();
        //applyCustomAttributes(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float coeffX = ((float) getWidth()) / ((float) imag.length);
        float coeffY = ((float) getHeight()) / 256f;
        for (int i = 0; i < imag.length; i++) {
            canvas.drawPoint(i*coeffX, coeffY*(128 - imag[i]), this.paint);
        }
