...
public class VisualizerViewLineFftMagnitude extends View implements VisualizerView {
    //FIELDS
    protected int id;
    ...
    //CONSTRUCTORS
    public VisualizerViewLineFftMagnitude(Context context, float strokeWidth, int color) {
        super(context);
    ...
    public VisualizerViewLineFftMagnitude(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.doneGettingOriginalSize = false;
        init();
        //applyCustomAttributes(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < (magnitude.length-1); i++) {
            canvas.drawLine(i*coeffX, coeffY*(256 - magnitude[i]), (i+1)*coeffX, coeffY*(256 - magnitude[i+1]), this.paint);
        }
    ...
    private static float[] returnMagnitudeWithSymmetry(byte[] bytes) {
        int n = bytes.length;
        float[] magnitudes = new float[n];
        magnitudes[0] = magnitudes[n - 1] = (float) Math.abs(bytes[0]);      // DC
        magnitudes[n / 2] = (float) Math.abs(bytes[1]);  // Nyquist
        for (int k = 1; k < n / 2; k++) {
            int i = k * 2;
            magnitudes[k] = magnitudes[(n-1) - k] = (float) Math.hypot(bytes[i], bytes[i + 1]);     //module (form) operation
        }
        return magnitudes;
    }
