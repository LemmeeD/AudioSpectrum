...
public class VisualizerViewLineWaveform extends View implements VisualizerView {
    //FIELDS
    protected int id;
    ...
    //CONSTRUCTORS
    public VisualizerViewLineWaveform(Context context, float strokeWidth, int color) {
        super(context);
    ...
    public VisualizerViewLineWaveform(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.doneGettingOriginalSize = false;
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < bytes.length - 2; i++) {
            points[i*4] = coeffX*i;
            points[i*4+1] = getHeight()/2f + coeffY*((byte) (bytes[i] + 128));
            points[i*4+2] = coeffX*(i+1);
            points[i*4+3] = getHeight()/2f + coeffY*((byte) (bytes[i+1] + 128));
        }
