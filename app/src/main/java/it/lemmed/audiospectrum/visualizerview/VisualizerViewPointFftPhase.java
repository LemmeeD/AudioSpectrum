...
public class VisualizerViewPointFftPhase extends View implements VisualizerView {
    //FIELDS
    protected int id;
    ...
    //CONSTRUCTORS
    public VisualizerViewPointFftPhase(Context context, float strokeWidth, int color) {
        super(context);
    ...
    public VisualizerViewPointFftPhase(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.doneGettingOriginalSize = false;
        init();
        //applyCustomAttributes(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float coeffX = ((float) getWidth()) / ((float) phase.length);
        float coeffY = ((float) getHeight()) / ((float) 4*((float) Math.PI));
        for (int i = 0; i < phase.length; i++) {
            canvas.drawPoint(i*coeffX, coeffY*(((float) 2*((float) Math.PI)) - phase[i]), this.paint);
        }
