...
public class PlayerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //this.graphic = preferences.getBoolean("key_visualization_graphic", false);
        ...
        LogDebug.log("playerActivity started with Settings as: visualWaveform="+visualizeWaveform+", visualFft="+visualizeFft+",Fft_m/a="+fftConf+", fftSize="+fftSize+", rate="+rate);
    }
    ...
    public void onConfigurationChanged(@NonNull Configuration newConfig) {
        ...
        if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
            if (this.player.isPlaying()) {
                ...
                this.player.stop();
                this.player.release();
                this.reInitialize(currentPosition);
            }
            else {
                this.visualizer.setEnabled(false);
                this.visualizer.setDataCaptureListener(null, this.rate, false, false);
                ...
                this.player.release();
                this.reInitialize(currentPosition);
                this.reInitialize(this.samplingRate);
            }
    ...
    //AUXILIARY METHODS
    @RequiresApi(api = Build.VERSION_CODES.N)
    private void reInitialize(int currentPosition) {
        setContentView(R.layout.activity_player);
        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar_player);
        setSupportActionBar(toolbar);
        ActionBar actionBar = getSupportActionBar();
        actionBar.setDisplayHomeAsUpEnabled(true);
        LogDebug.log("--- PlayerActivity ---");
        //LogDebug.log("ConfigChange: player audio session id: "+this.player.getAudioSessionId()+", and it is playing: "+this.player.isPlaying());
        LogDebug.log("ConfigChange with: visualWaveform="+this.visualizeWaveform+", visualFft="+this.visualizeFft+",Fft_m/a="+this.fftConf+", fftSize="+this.fftSize+", rate="+this.rate);
        //firstStart = true;
        ...
        this.fftSize = Integer.parseInt(preferences.getString("key_capture_size", "1024"));
        this.rate = Integer.parseInt(preferences.getString("key_visualization_rate", "10000"));
        //this.samplingRate = activeSamplingRate;
        if (this.visualizeWaveform) {
            //Waveform graph
            this.graph1 = findViewById(R.id.graph_1);
            GraphViewUtils.initWaveformGraph(graph1, this.fftSize, this.samplingRate);
            series1 = new LineGraphSeries<>();
            series1.setColor(colorWaveform);
            //series1.setThicknesssss..
            graph1.addSeries(series1);
        }
        if (this.visualizeFft) {
            if (this.fftConf) {
                ...
                this.graph2 = findViewById(R.id.graph_2);
                this.graph3 = findViewById(R.id.graph_3);
                GraphViewUtils.initFftMagnitudeGraph(graph2, fftSize, samplingRate);
                GraphViewUtils.initFftPhaseGraph(graph3, fftSize, samplingRate);
                series2 = new LineGraphSeries<>();
                series2.setColor(colorFft2);
                series3 = new LineGraphSeries<>();
                series3.setColor(colorFft3);
                ...
                this.graph2 = findViewById(R.id.graph_2);
                this.graph3 = findViewById(R.id.graph_3);
                GraphViewUtils.initFftRealGraph(graph2, fftSize, samplingRate);
                GraphViewUtils.initFftImagGraph(graph3, fftSize, samplingRate);
                series2 = new LineGraphSeries<>();
                series2.setColor(colorFft2);
                series3 = new LineGraphSeries<>();
                series3.setColor(colorFft3);
        ...
        this.visualizer = new Visualizer(this.player.getAudioSessionId());
        this.visualizer.setCaptureSize(fftSize);
        this.visualizer.setDataCaptureListener(new PlayerOnDataCaptureListener(graph1, graph2, graph3, series1, series2, series3, fftConf), this.rate, visualizeWaveform, visualizeFft);
        samplingRate = this.visualizer.getSamplingRate();
